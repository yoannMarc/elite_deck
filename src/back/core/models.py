...
@dataclass(slots=True)
class CombatTarget:
    """Vaisseau actuellement ciblé (données progressives selon ScanStage)."""
    locked: bool = False
    ship: str = ""
    pilot: str = ""
    rank: str = ""
    shield_health: float = 0.0
    hull_health: float = 0.0
    faction: str = ""
    legal_status: str = ""
    bounty: int = 0
    scan_stage: int = 0
...
@dataclass(slots=True)
class CombatState:
    """État de combat reconstruit depuis les événements du Journal."""
    # État du vaisseau en combat
    shields_up: bool = True
    under_attack: bool = False
    interdicted: bool = False

    # Cible actuelle
    target: CombatTarget = field(default_factory=CombatTarget)

    # Dernière prime collectée
    last_bounty_total: int = 0
    last_bounty_target: str = ""
    last_bounty_victim_faction: str = ""
    last_bounty_rewards: list[BountyReward] = field(default_factory=list)

    # Statistiques de session (remises à zéro à LoadGame)
    kills_session: int = 0
    bonds_session: int = 0
    bonds_total_session: int = 0
